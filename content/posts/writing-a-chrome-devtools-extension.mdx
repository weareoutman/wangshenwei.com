...
我们重新回到之前遇到的问题：Eval 接口只能传递可以被 JSON 序列化的数据。对此我们不能责怪 Chrome，序列化是很多通讯方式的基础，遥远如书信，信息被序列化为文字，近如网络，信息被序列化为数字信号。序列化的过程是有损的，原始对象序列化再反序列化后的得到的对象其实不是真正的原始对象，最多是一个拷贝，例如电话里我们听到的声音是手机扬声器发出的，而不是真正的人声，网络视频里我们看到的是屏幕绘制的影像而不是真实的世界。
