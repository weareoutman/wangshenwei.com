...
这看起来可以让我们拿到想要的渲染树信息了。但是官方文档也说明了它对代码执行结果的要求：*合法的 JSON 对象*，即可以序列化为 JSON 的对象，它只能包含 JavaScript 原始类型的值、并且不包含循环引用。这意味着它的能力受到了限制，例如无法直接传递原生事件对象 `Event`、`CustomEvent` 或者内部包含重复引用的属性，不过我们先暂时搁置这个潜在的问题。
...
渲染树可以显示了，那么如何审查单个自定义元素呢？Eval 接口接收的是一串字符串代码，无法为它传递引用，但我们可以想办法在 `hook.js` 中存储一个映射表来存放自增 ID 与自定义元素的关系，Eval 调用时只传递 ID。
...
在这个通信过程中，我们再次遇到了和 Eval 接口一样的问题：只能传递 JSON 序列化的数据。序列化是很多通讯方式的基础，遥远如书信，信息被序列化为文字，近如网络，信息被序列化为数字信号。序列化再反序列化后的得到的信息其实不是真正的原始信息，而是一份有损的拷贝，正如电话里听到的是手机扬声器还原的对方的声音，网络视频里看到的是屏幕上绘制的对方镜头的影像拷贝。

那如何解决 JSON 序列化无法支持的重复/循环引用和复杂对象类型信息呢？再次参考 React Developer Tools 的源码，它使用了一对命名为 `dehydrate` 和 `hydrate` （脱水和补水）的函数，发送消息前，`dehydrate` 将原始信息进行*脱水*，将复杂对象类型转换为特定的可以被 JSON 序列化的结构信息，接收消息时，对数据进行*补水*，还原得到原始的对象类型。

这对函数的命名有点意思，让我想到了科幻小说《三体》里的情节，三体人为了在乱纪元中生存，会自我*脱水*进入休眠状态，而到了恒纪元则完成*补水*重新苏醒。这或许正如复杂数据想要在通信中保持相对完整也需要进行脱水和补水一样。

回到主题，React Developer Tools 的实现中没有包含对重复/循环引用的处理，它通过限制递归层级绕开了这个问题。
