...
这看起来可以让我们拿到想要的渲染树信息了。但是官方文档也说明了它对代码执行结果的要求：*合法的 JSON 对象*，即可以序列化为 JSON 的对象，它只能包含 JavaScript 原始类型的值、并且不包含循环引用。这意味着它的能力受到了限制，例如无法直接传递原生事件对象 `Event`、`CustomEvent` 或者内部包含循环引用的属性，不过我们先暂时搁置这个潜在的问题。
...
> 注：本文代码均只作示意用途，非实际代码。该扩展程序目前还在 Chrome Web Store 审核中，源码托管在 GitHub：[brick-next-devtools]。
